...
final class UpdateRouter
{
    ...
    /** @var array<string, list<callable>> Обработчики callback_data. */
    private array $callbackDataHandlers = [];

    /** @var list<callable> Обработчики callback_query. */
    ...
    {
        $this->callbackQueryHandlers[] = $handler;

        return $this;
    }

    public function onCallbackData(string $data, callable $handler): self
    {
        $this->callbackDataHandlers[$data][] = $handler;
    ...
    public function dispatch(Update $update, BotContext $context): bool
    {
        if ($update->isCallbackQuery()) {
            ...
            if ($callbackData !== null) {
                $handlers = $this->callbackDataHandlers[$callbackData] ?? [];
                if ($handlers !== []) {
            ...
            if ($this->callbackQueryHandlers !== []) {
                foreach ($this->callbackQueryHandlers as $handler) {
                    $handler($update, $context);
                }
